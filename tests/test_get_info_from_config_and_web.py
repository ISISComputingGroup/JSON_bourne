...
class TestGetInfoFromConfigAndWeb(unittest.TestCase):
    ...
    def test_GIVEN_run_duraction_WHEN_parse_THEN_correctly_formatted_duration_returned(self):
        ...
        self.reader.get_json_from_instrument_archive = Mock(
            return_value=ArchiveMother.create_info_page(
                [ArchiveMother.create_channel(name=name, value=value, units=units)]
            )
        )
    ...
    def test_GIVEN_display_set_to_something_weird_WHEN_parse_THEN_title_and_user_appear_as_unavailable(
        self,
    ):
        title_name = "DAE:TITLE.VAL"
        title_value = "a title"
    ...
    def test_GIVEN_display_is_not_present_WHEN_parse_THEN_title_and_user_appear_as_unavailable(
        self,
    ):
        title_name = "DAE:TITLE.VAL"
        title_value = "a title"
    ...
    def test_GIVEN_visible_block_WHEN_parse_THEN_block_is_marked_as_visible(self):
        ...
        assert_that(
            result["groups"][group_name][block_name]["visibility"], is_(expected_is_visible)
        )
