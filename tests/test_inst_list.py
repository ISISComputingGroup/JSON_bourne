...
class TestInstList(unittest.TestCase):
    ...
    def test_GIVEN_caget_contains_one_instrument_WHEN_retrive_THEN_list_returned(self):
        ...
        host_name = "NDXINST"
        prefix = "IN:INST:"
        caget_value = self.compress_and_hex(
            json.dumps([{"pvPrefix": prefix, "hostName": host_name, "name": instrument}])
        )
    ...
    def test_GIVEN_caget_throws_not_connected_and_no_cached_values_WHEN_retrive_THEN_empty_cached_list_returned(
        self,
    ):
        global caget_value, caget_error
    ...
    def test_GIVEN_caget_contains_non_hex_string_WHEN_retrive_THEN_empty_cached_list_returned(self):
        ...
        assert_that(list, is_({}))
        assert_that(
            self.inst_list.error_on_retrieve, is_(InstList.INSTRUMENT_LIST_NOT_DECOMPRESSED)
        )

    def test_GIVEN_caget_contains_non_compresed_string_WHEN_retrive_THEN_empty_cached_list_returned(
        self,
    ):
        global caget_value, caget_error
    ...
    def test_GIVEN_caget_contains_non_json_inst_list_WHEN_retrive_THEN_empty_cached_list_returned(
        self,
    ):
        global caget_value, caget_error
    ...
    def test_GIVEN_caget_contains_nonsense_inst_list_WHEN_retrive_THEN_empty_cached_list_returned(
        self,
    ):
        global caget_value, caget_error

        caget_value = self.compress_and_hex(
            json.dumps([{"pvPrefix": "IN:INST:", "hostName": "host"}])
        )
        ...
        assert_that(list, is_({}))
        assert_that(
            self.inst_list.error_on_retrieve, is_(InstList.INSTRUMENT_LIST_NOT_CORRECT_FORMAT)
        )

    def test_GIVEN_caget_contains_instrument_then_has_error_WHEN_retrive_THEN_first_cached_list_returned(
        self,
    ):
        global caget_value, caget_error
