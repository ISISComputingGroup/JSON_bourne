...
class TestHandlerUtils_InstrumentandCallback(unittest.TestCase):
    ...
    def test_GIVEN_path_with_callback_and_no_instrument_WHEN_get_instrument_and_callback_called_THEN_raises_error(
        self,
    ):
        with self.assertRaises(ValueError):
            get_instrument_and_callback(CALLBACK_STR.format("test"))

    def test_GIVEN_path_with_instrument_and_no_callback_WHEN_get_instrument_and_callback_called_THEN_raises_error(
        self,
    ):
        with self.assertRaises(ValueError):
            get_instrument_and_callback(INST_STR.format("test"))

    def test_GIVEN_path_with_callback_and_empty_instrument_WHEN_get_instrument_and_callback_called_THEN_raises_error(
        self,
    ):
        with self.assertRaises(ValueError):
            get_instrument_and_callback(CALLBACK_AND_INST.format("test", ""))

    def test_GIVEN_path_with_instrument_and_empty_callback_WHEN_get_instrument_and_callback_called_THEN_raises_error(
        self,
    ):
        with self.assertRaises(ValueError):
            get_instrument_and_callback(CALLBACK_AND_INST.format("", "test"))

    def test_GIVEN_path_with_multiple_callbacks_WHEN_get_instrument_and_callback_called_THEN_raises_error(
        self,
    ):
        with self.assertRaises(ValueError):
            get_instrument_and_callback(
                CALLBACK_STR.format("test") + CALLBACK_AND_INST.format("test", "test")
            )

    def test_GIVEN_path_with_multiple_instruments_WHEN_get_instrument_and_callback_called_THEN_raises_error(
        self,
    ):
        with self.assertRaises(ValueError):
            get_instrument_and_callback(
                CALLBACK_AND_INST.format("test", "test") + INST_STR.format("test")
            )

    def test_GIVEN_path_with_callback_WHEN_get_instrument_and_callback_called_THEN_expected_callback_returned(
        self,
    ):
    ...
    def test_GIVEN_path_with_upper_instrument_WHEN_get_instrument_and_callback_called_THEN_expected_instrument_returned(
        self,
    ):
        exp_instrument = "INSTRUMENT"
        inst, callback = get_instrument_and_callback(
            CALLBACK_AND_INST.format("test", exp_instrument)
        )
        self.assertEqual(exp_instrument, inst)

    def test_GIVEN_path_with_lower_instrument_WHEN_get_instrument_and_callback_called_THEN_upper_instrument_returned(
        self,
    ):
        exp_instrument = "instrument"
        inst, callback = get_instrument_and_callback(
    ...
    def test_GIVEN_path_with_instrument_containing_hyphen_WHEN_get_instrument_and_callback_called_THEN_upper_instrument_returned(
        self,
    ):
...
class TestHandlerUtils_IbexRunning(unittest.TestCase):
    ...
    def test_GIVEN_dict_with_key_containing_data_WHEN_get_ibex_running_called_THEN_running_instrument_true(
        self,
    ):
        inst = "TEST"
        inp = {inst: "some_data"}
    ...
    def test_GIVEN_dict_with_key_containing_no_data_WHEN_get_ibex_running_called_THEN_running_instrument_false(
        self,
    ):
        inst = "TEST"
        inp = {inst: ""}
    ...
    def test_GIVEN_dict_with_instruments_with_and_without_data_WHEN_get_ibex_running_called_THEN_running_and_not_running_instruments_returned(
        self,
    ):
    ...
    def test_GIVEN_dict_with_run_state_of_running_WHEN_get_summary_details_THEN_run_state_is_running(
        self,
    ):
        inst = "TEST"
    ...
    def test_GIVEN_dict_with_no_run_state_as_none_WHEN_get_summary_details_THEN_run_state_is_unknown(
        self,
    ):
        inst = "TEST"
    ...
    def test_GIVEN_dict_with_no_run_state_missing_value_key_WHEN_get_summary_details_THEN_run_state_is_unknown(
        self,
    ):
        inst = "TEST"
        expected_state = "UNKNOWN"
    ...
    def test_GIVEN_dict_with_no_run_state_value_WHEN_get_summary_details_THEN_run_state_is_unknown(
        self,
    ):
        inst = "TEST"
        expected_state = "UNKNOWN"
    ...
    def test_GIVEN_dict_with_multiple_instruments_WHEN_get_summary_details_THEN_instruments_returned_in_named_order(
        self,
    ):
        inst = "TEST"
...
class TestHandlerUtils_DetailedInstrumentState(unittest.TestCase):
    ...
    def test_GIVEN_instrument_with_data_WHEN_get_details_called_THEN_data_returned_for_that_instrument(
        self,
    ):
        inst = "TEST_INST"
        inst_data = ["test"]
...
class TestHandlerUtils_InstrumentTime(unittest.TestCase):
    ...
    def test_that_GIVEN_wrong_formated_instrument_THEN_raise_value_error(self):
        ...
        assert_that(
            calling(get_instrument_time_since_epoch).with_args("", instrument_data),
            raises(ValueError),
        )

    def test_that_GIVEN_instrument_data_without_time_of_day_THEN_raise_value_error(self):
        instrument_data = {
            "inst_pvs": {"this_is_not_TIME_OF_DAY": {"value": "01/01/1970 01:00:00"}}
        }
...
class TestHandlerUtils_CheckOutOfSync(unittest.TestCase):
    def test_that_GIVEN_time_difference_is_greater_than_threshold_THEN_set_out_of_sync_to_true(
        self,
    ):
