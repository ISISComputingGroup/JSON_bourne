...
class TestWebScrapperManager(unittest.TestCase):
    def setUp(self):
        ...
        self.inst_list = MockInstList(
            {self.expected_name: (self.expected_host, self.expected_prefix)}
        )
        self.reader = Mock()
    ...
    def test_GIVEN_one_instruments_on_list_which_already_has_a_scrapper_WHEN_run_THEN_no_web_scrapper_created(
        self,
    ):
        web_scrapper_manager = WebScrapperManager(MockWebScrapper, self.inst_list)
        web_scrapper_manager.maintain_scrapper_list()
    ...
    def test_GIVEN_no_instruments_on_list_but_scrapper_exists_WHEN_run_THEN_web_scrapper_stopped_and_removed(
        self,
    ):
    ...
    def test_GIVEN_one_instruments_on_list_which_has_same_host_name_as_a_scrapper_but_different_name_WHEN_run_THEN_only_one_new_scrapper_running(
        self,
    ):
        different_name = "diff"
        ...
        mock_inst_list.instrument_host_dict = {
            self.expected_name: (self.expected_host, self.expected_prefix)
        }
    ...
    def test_GIVEN_one_instruments_on_list_which_has_same_name_as_a_scrapper_but_different_host_WHEN_run_THEN_new_web_scrapper_created(
        self,
    ):
        different_host = "diff"
    ...
    def test_GIVEN_one_instruments_on_list_which_already_has_a_scrapper_but_scrapper_is_not_alive_WHEN_run_THEN_new_web_scrapper_created(
        self,
    ):
        web_scrapper_manager = WebScrapperManager(MockWebScrapper, self.inst_list)
        web_scrapper_manager.maintain_scrapper_list()
